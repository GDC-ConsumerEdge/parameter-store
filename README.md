...
### Prepare DNS name of Prameter Store app
...
> ```
### SSL Certificate
Once the DNS name is decided, we need to acquire a SSL cert for the Parameter Store App.  
...
## Build Image
We will build a docker image and push to an artifact registry in your deployment GCP project. Please make sure the artifact registry is available and your account has sufficient permisson to push image to it. 
...
* For better collaboration, most of the time we need to save the terraform state file to a shared place, e.g. a GCP bucket. To achieve this, we need to configure a GCP bucket as terraform backend.
  1. Create or using a exsting GCS bucket. Make sure your current account has write permission to it.
...
This list should include the DNS name you assigned to your Parameter Store app, as stated in [Prepare DNS name of Prameter Store app](#prepare-dns-name-of-prameter-store-app)
...
This final step must be done via **GCP Console**
1. Go to **Security** &rarr; **Idetity-Aware Proxy**
